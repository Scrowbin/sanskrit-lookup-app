...
def run_focused_benchmark(csv_file="verbs_clean.csv", output_report="benchmark_failures.csv"):
    ...
    with open(csv_file, mode='r', encoding='utf-8') as f:
        for row in csv.DictReader(f):
            key = (
                ...
                "denominative" if row.get('class') == 'denom'
    ...
    for root, primary_class, desc in test_suite:
        ...
        derivations = ["denominative"] if is_denom else ["primary", "causative", "desiderative", "intensive"]
                    ...
                    for person in ALL_PERSONS:
                        for number in ALL_NUMBERS:
                            ...
                            try:
                                ...
                                call_kwargs = dict(voice=voice, tense=tense,
                                                   derivative=fst_derivative, use_db=False)
                                if is_denom:
                                    actual = api.conjugate(engine_root, effective_class, person, number, **call_kwargs)
                                else:
        ...
        pct_root    = f"{100 * counts['pass'] / tested_root:.0f}%" if tested_root else "n/a"
        print(f"  ✔ {root:<10} {pct_root:>4}  "
