...
ALPHA = pn.union(
    *"abcdefghijklmnopqrstuvwxyzāīūṛṝḷḹeaiouḥṃṅñṇnṃśṣs"
)  # UNFINISHED, DIS DA ALPHABET
"""
VOWELS
"""
...
voiced_consonants = pn.union(voiced_stops, nasals, semivowels, aspirate)
"""HARD CONSONANTS"""
# Unvoiced unaspirated and aspirated stops
unvoiced_stops = pn.union("k", "kh", "c", "ch", "ṭ", "ṭh", "t", "th", "p", "ph")
# Sibilants (fricatives)
sibilants = pn.union("ś", "ṣ", "s")
hard_consonants = pn.union(unvoiced_stops, sibilants)
...
all_consonants = pn.union(
    guttural, palatal, retroflex, dental, labial, semivowels, sibilants, aspirate
)
...
apply_nati = pn.cdrewrite(
    pn.cross("n", "ṇ"), triggers + allowed_interveners, right_context, ALPHA
)
