...
visarga_sigma = pn.closure(pn.union(phonology.sigma, "ḥ"))
visarga = "ḥ"
"""
...
visarga_before_a_after_not_a = pn.cdrewrite(
    ...
    pn.cross("ḥ", ""),
    pn.union("a", "ā"),
    pn.difference(phonology.vowels, pn.union("a")),
    visarga_sigma,
)
...
visarga_before_vowel_or_consonant = pn.cdrewrite(
    ...
    pn.cross("ḥ", "r"),
    "",
    pn.difference(phonology.vowels, phonology.voiced_consonants),
    visarga_sigma,
)
...
visarga_before_c = pn.cdrewrite(
    pn.cross(visarga, "ś"), "", pn.union("c", "ch"), visarga_sigma
)

# before ṭ ṭh → ṣ
visarga_before_t_dot = pn.cdrewrite(
    pn.cross(visarga, "ṣ"), "", pn.union("ch", "ṭ"), visarga_sigma
)
# before t th → s
visarga_before_t_th = pn.cdrewrite(
    pn.cross(visarga, "s"), "", pn.union("ch", "t"), visarga_sigma
)


# s_to_visarga
s_to_visarga = pn.cdrewrite(pn.cross("s", visarga), "", "<EOS>", visarga_sigma)
