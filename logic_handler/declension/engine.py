...
from a_stem_rules import (
    masc_a_stem_paradigm, fem_a_stem_paradigm, neut_a_stem_paradigm,
)
...
class DeclensionEngine:
    ...
    def _detect_stem_tag(self, stem: str, gender_tag: str) -> str:
        ...
        if gender_tag == "[Fem]" and stem.endswith("ā"):
            return "[Ā_STEM]"
        if stem.endswith("ī"):   return "[I_bar_STEM]"
        if stem.endswith("ū"):   return "[Ū_STEM]"
        if stem.endswith("vāṅs"): return "[VAS_STEM]"
        if stem.endswith("vant"): return "[VANT_STEM]"
        if stem.endswith("mant"): return "[MANT_STEM]"
        if stem.endswith("ant"):  return "[ANT_STEM]"
        if stem.endswith("in"):   return "[IN_STEM]"
        if stem.endswith("an"):   return "[AN_STEM]"
        if stem.endswith("as"):   return "[AS_STEM]"
        if stem.endswith("is"):   return "[IS_STEM]"
        if stem.endswith("us"):   return "[US_STEM]"
        if stem.endswith("au"):   return "[AU_STEM]"
        if stem.endswith("ai"):   return "[AI_STEM]"
        if stem.endswith("e"):    return "[E_STEM]"
        if stem.endswith("o"):    return "[O_STEM]"
        if stem.endswith("i"):    return "[I_STEM]"
        if stem.endswith("u"):    return "[U_STEM]"
        if stem.endswith("ṛ") or stem.endswith("ṝ"): return "[R_STEM]"
    ...
    _JUNC_VOICE: dict[str, str] = {       # before voiced sounds (bh/b/g/d…)
        "kh": "g",  "k": "g",
        "ch": "g",  "c": "g",  "jh": "g",  "j": "g",   # palatals → velar g
        "ṭh": "ḍ",  "ṭ": "ḍ",  "ḍh": "ḍ",
        "th": "d",  "t": "d",  "dh": "d",
        "ph": "b",  "p": "b",  "bh": "b",
        "ś":  "j",  "h": "gh",
    ...
    _JUNC_DEVOICE: dict[str, str] = {     # before voiceless sounds (s/k/t/p)
        "gh": "k",  "g": "k",
        "jh": "k",  "j": "k",  "ch": "k",  "c": "k",   # palatals → velar k
        "ḍh": "ṭ",  "ḍ": "ṭ",  "ṭh": "ṭ",
        "dh": "t",  "d": "t",  "th": "t",
        "bh": "p",  "b": "p",  "ph": "p",
        "ś":  "k",  "ṣ": "ṭ",  "h": "k",
    ...
    def _junction_sandhi(self, form: str) -> str:
        ...
        while "#" in form:
            ...
            elif after[:1] in ("s", "ś", "ṣ") or after.startswith("k") or after.startswith("t"):
    ...
    _PERMITTED: dict[str, str] = {
        "gh": "k",  "kh": "k",  "g": "k",
        "jh": "k",  "ch": "k",  "j": "k",  "c": "k",   # palatals → velar
        "ḍh": "ṭ",  "ṭh": "ṭ",  "ḍ": "ṭ",
        "dh": "t",  "th": "t",  "d": "t",
        "bh": "p",  "ph": "p",  "b": "p",
        "ś":  "ṭ",  "ṣ":  "ṭ",
    ...
    def _query(
        ...
        """Compose *annotated* through the master FST and post-process."""
        try:
            result_fst = (pn.accep(annotated) @ self._master).optimize()
            raw_forms: list[str] = list(result_fst.paths().ostrings())
        except Exception:
            return []
    ...
    def declense(
        ...
        if not gender_tag:
            raise ValueError(
                f"Unknown gender {gender!r}. Use 'm'/'f'/'n' "
                "(or 'masc'/'fem'/'neut')."
            )
        ...
        if tag == "[R_STEM]":
            gender_tag = {"agt": "[Agt]", "kin": "[Kin]", "neut": "[Neut]"}.get(
                r_subtype, "[Agt]"
            )
        ...
        for case in CASES:
            for number in NUMBERS:
                annotated = f"{stem}{tag}{gender_tag}[{case}][{number}]"
                results[(case, number)] = self._query(annotated, case, number, gender_tag)
