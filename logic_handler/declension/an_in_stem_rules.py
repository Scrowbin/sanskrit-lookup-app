...
loc_pl_in = in_cons_base + pn.cross(
    "[Loc][Pl]", "su"
)  # yogin -> yogisu (Strict underlying s)
...
nom_du_in = (
    in_vow_base + pn.cross("[Nom][Du]", "au")
)  # yogin -> yoginau (Masc) / yoginī (Neut covered by standard 'ī' rule if mapped separately, chart shows yoginī)
...
voc_sg_an_n = pn.cross("an[AN_STEM][Neut][Voc][Sg]", "a")
...
ins_sg_an = an_vow_base + pn.cross(
    "[Ins][Sg]", "ā"
)  # rājan -> rājanā (Sandhi will make rājñā)
...
nom_du_an_n = an_vow_base + pn.cross(
    "[Nom][Du]", "ī"
)  # nāman -> nāmanī (Sandhi will make nāmnī)
...
acc_pl_an_m = an_vow_base + pn.cross(
    "[Acc][Pl]", "as"
