...
ins_sg_masc_i = pn.cross(
    "[I_STEM][Ins][Sg]", "nā"
)  # agni + [Ins][Sg] -> agninā (appends)
dat_sg_masc_i = pn.cross(
    "i[I_STEM][Dat][Sg]", "aye"
)  # agni + [Dat][Sg] -> agnaye (i -> aye)
...
ins_sg_fem_i = pn.cross(
    "i[I_STEM][Ins][Sg]", "yā"
)  # gati + [Ins][Sg] -> gatyā (i -> yā)
...
abl_sg_fem_i = pn.union(
    pn.cross("i[I_STEM][Abl][Sg]", "es"), pn.cross("i[I_STEM][Abl][Sg]", "yās")
)
gen_sg_fem_i = pn.union(
    pn.cross("iI_STEM][Gen][Sg]", "es"), pn.cross("i[I_STEM][Gen][Sg]", "yās")
)
...
nom_sg_neut_i = pn.cross(
    "[I_STEM][Nom][Sg]", ""
)  # vāri + [Nom][Sg] -> vāri (no change)
...
nom_pl_i = pn.cross(
    "i[I_STEM][Nom][Pl]", "ayas"
)  # agni + [Nom][Pl] -> agnayas (i -> ayas)
...
ins_pl_i = pn.cross(
    "[I_STEM][Ins][Pl]", "bhis"
)  # agni + [Ins][Pl] -> agnibhis (appends)
dat_pl_i = pn.cross("[I_STEM][Dat][Pl]", "bhyas")  # agni + [Dat][Pl] -> agnibhyas
abl_pl_i = pn.cross("[I_STEM][Abl][Pl]", "bhyas")  # agni + [Abl][Pl] -> agnibhyas

gen_pl_i = pn.cross(
    "i[I_STEM][Gen][Pl]", "īnām"
)  # agni + [Gen][Pl] -> agnīnām (i -> īnām)
loc_pl_i = pn.cross(
    "[I_STEM][Loc][Pl]", "su"
)  # agni + [Loc][Pl] -> agniṣu (appends ṣu)

# Neuter plural i-stem (e.g., vāri)
nom_pl_neut_i = pn.cross(
    "i[I_STEM][Nom][Pl]", "īni"
)  # vāri + [Nom][Pl] -> vārīni (i -> īni)
...
gen_pl_neut_i = pn.cross(
    "i[I_STEM][Gen][Pl]", "īnām"
)  # vāri + [Gen][Pl] -> vārīnām (i -> īnām)
...
"""
# Singular
nom_sg_f_ii = pn.cross(
    "ī[I_bar_STEM][Fem][Nom][Sg]", "ī"
)  # devī -> devī (Drops the 's'!)
...
voc_sg_f_ii = pn.cross(
    "ī[I_bar_STEM][Fem][Voc][Sg]", "i"
)  # devī -> devi (Shortens to i!)
...
loc_pl_f_ii = pn.cross(
    "[I_bar_STEM][Fem][Loc][Pl]", "su"
)  # devī + su (Strict underlying)
